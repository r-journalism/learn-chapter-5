...
This is from the [fifth chapter](http://learn.r-journalism.com/en/mapping/census_maps/census-maps/) of [learn.r-journalism.com](https://learn.r-journalism.com/).
...
You could download and import the shapefile into R [yourself](/mapping/static_maps/static-maps/index.html), but there's a package that brings in Census shapefiles for you called [**Tigris**](https://github.com/walkerke/tigris).
...
```{r viz_states, fig.width=8, fig.height=4}
states %>% 
  leaflet() %>% 
  addTiles() %>% 
...
We've already gone in some detail into how to use this package in th eprevious section.
...
```{r finalmap1, fig.width=8, fig.height=4, warning=F, message=F}
leaflet() %>%
